...
import ButtonGetStrated from "../Components/Button";

const navigation = [
  { name: "More About Us", href: "Hero" },
  { name: "Our Works", href: "" },
...
function Header() {
  ...
  return (
    <header className="absolute inset-x-0 top-0 z-50 pb-18">
      <nav
        aria-label="Global"
      ...
      >
        <div className="flex lg:flex-1 ">
          <a href="#" className="-m-1.5 p-1.5">
            <span className="sr-only">MediaCraft Studios</span>
        ...
        <div className="hidden lg:flex lg:gap-x-12">
          {navigation.map((item) => (
            <a
              key={item.name}
              href={item.href}
              className="text-sm/6 font-semibold text-white hover:text-yellow-600"
            >
              {item.name}
            </a>
          ))}
